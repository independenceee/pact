 ...
 * @function submitTx
 * @description
 * Handles submission of a signed Cardano transaction to the blockchain
 * using the Blockfrost provider, and waits until the transaction is confirmed.
 *
 * This function:
 * 1. Submits the transaction to the network.
 * 2. Waits for Blockfrost to confirm that the transaction was included in a block.
 * 3. Returns the transaction hash and success status once confirmed.
 *
 * @param {Object} params - Function parameters.
 * @param {string} params.signedTx - The signed transaction in CBOR hexadecimal format.
 *
 * @returns {Promise<{ data: string | null; result: boolean; message: string }>}
 * - `data`: Transaction hash if successful, otherwise `null`.
 * - `result`: `true` if submission and confirmation succeed, `false` if failed.
